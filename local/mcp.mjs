...
if (fs.existsSync(envFile)) for (const l of fs.readFileSync(envFile,'utf8').split('\n')) {
  const mm = l.match(/^\s*([A-Z0-9_]+)\s*=\s*(.*)\s*$/); if (mm && !process.env[mm[1]]) process.env[mm[1]] = mm[2];
}
const TOKEN = process.env.CHANNEL_ACCESS_TOKEN || '';
const DEFAULT_USER = process.env.DESTINATION_USER_ID || '';
// remote inbox API (receiver runs on the Pi; reached through the Cloudflare tunnel)
const API_URL = (process.env.INBOX_API_URL || 'http://127.0.0.1:18081').replace(/\/$/, '');
const API_TOKEN = process.env.INBOX_API_TOKEN || process.env.API_TOKEN || '';
const api = (p, opts = {}) => fetch(API_URL + p, { ...opts, headers: { 'x-api-token': API_TOKEN, 'Content-Type': 'application/json', ...(opts.headers || {}) } });
// heartbeat: while this MCP process lives, Claude counts as online (receiver skips auto-reply)
const beat = () => { api('/heartbeat', { method: 'POST' }).catch(() => {}); };
beat(); setInterval(beat, 30_000).unref();
for (const sig of ['SIGINT','SIGTERM','SIGHUP']) process.on(sig, () => process.exit(0));
const load = async () => { const r = await api('/inbox'); if (!r.ok) throw new Error(`inbox api ${r.status}`);
  return (await r.text()).split('\n').filter(Boolean).map(l=>JSON.parse(l)); };
const ack = async (ids) => { const r = await api('/ack', { method: 'POST', body: JSON.stringify(ids ? { ids } : {}) });
  return r.ok ? (await r.json()).marked : 0; };
const summarize = (e) => ({
...
server.tool('get_line_messages', 'Read messages/events received from LINE users via the webhook inbox. Defaults to unread only.',
  ...
  async ({ unread_only, limit, mark_read }) => {
    const all = await load(); let sel = unread_only ? all.filter(e => !e.read) : all; sel = sel.slice(-limit);
    if (mark_read && sel.length) await ack(sel.map(e=>e.webhookEventId).filter(Boolean));
...
server.tool('ack_line_messages', 'Mark LINE inbox events as read (all unread, or specific event ids).',
  ...
  async ({ ids }) => { const n = await ack(ids || null);
...
server.tool('line_inbox_status', 'Show inbox counts and whether the webhook receiver is running.', {},
  async () => { let up=false, all=[]; try { up = (await fetch(API_URL + '/health')).ok; all = await load(); } catch {}
    return { content: [{ type:'text', text: JSON.stringify({ receiver_up: up, api_url: API_URL, total: all.length, unread: all.filter(e=>!e.read).length, last: all.at(-1)?.receivedAt ?? null }) }] }; });
